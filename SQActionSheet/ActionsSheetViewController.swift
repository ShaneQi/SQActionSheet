...
final class ActionsSheetViewController: UIViewController {
	...
	static func getInstance(sheetTitle: String?, cancelText: String?, actions: [Action]) -> Self {
		let viewController = getInstance()
...
extension ActionsSheetViewController: StoryboardInstantiatable {}
...
extension ActionsSheetViewController: UITableViewDataSource {
	...
	func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		...
		case .title:
			let thisCell: ActionsSheetTitleCell = tableView.cook.dequeueReusableCell(for: indexPath)
			thisCell.titleLabel.text = sheetTitle
			cell = thisCell
		case .cancel:
			let thisCell: ActionsSheetCancelCell = tableView.cook.dequeueReusableCell(for: indexPath)
			thisCell.titleLabel.text = cancelText
			cell = thisCell
		case .action(let index):
			let action = actions[index]
			if case .destructive = action.type {
				let thisCell: ActionsSheetDestructiveCell = tableView.cook.dequeueReusableCell(for: indexPath)
				thisCell.iconImageView.image = action.image
				thisCell.titleLabel.text = action.text
				cell = thisCell
			} else {
				let thisCell: ActionsSheetDefaultCell = tableView.cook.dequeueReusableCell(for: indexPath)
				thisCell.iconImageView.image = action.image
				thisCell.titleLabel.text = action.text
				cell = thisCell
			}
